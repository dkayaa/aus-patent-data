...
- **IPC justification** vs seed `output`: teacher-written gold, so teacher vs gold is self-agreement; GPT/students matching that wording is not independent quality. Few-shot gold has the same format leak.
...
| Leak | Rule |
|------|------|
| SFT | Same `application_number` is never in both train and test (assignment is global across IPC / abstract). |
...
# --system qwen/qwen-2.5-7b-instruct --task abstract_drafting --limit 5
```

Resume via `done_ids.txt` under each prediction dir. `max_tokens`: 1024 for IPC and abstract.
...
**MRC** is retired from this stage (no longer split, baselined, or prepared for SFT). Legacy `mrc/` holdings may still exist under older generator dirs.
